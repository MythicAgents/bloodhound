...
class GetObjectArguments(TaskArguments):
...
class GetObject(CommandBase):
    cmd = "get_object"
    needs_admin = False
    help_cmd = "get_object -object_id \"S-1-5-21-909015691-3030120388-2582151266-512\""
    description = "Get information about a specific object"
    version = 1
    author = "@its_a_feature_"
    argument_class = GetObjectArguments
    supported_ui_features = ["bloodhound:get_object"]
    ...
    async def create_go_tasking(self,
        ...
        uri = f"/api/v2/base/{taskData.args.get_arg('object_id')}"
        try:
