...
class GetDomainsForeignUsersArguments(TaskArguments):
    ...
    def __init__(self, command_line, **kwargs):
        ...
        self.args = [
            CommandParameter(
                name="object_id",
                description="Which domain object_id to query to see if there are foreign users in groups in that domain",
...
class GetDomainsForeignUsers(CommandBase):
    cmd = "get_domains_foreign_users"
    needs_admin = False
    help_cmd = "get_domains_foreign_users -object_id \"S-1-5-21-909015691-3030120388-2582151266\""
    description = "Get information about foreign users in groups within the specified domain"
    version = 1
    author = "@its_a_feature_"
    argument_class = GetDomainsForeignUsersArguments
    supported_ui_features = ["bloodhound:get_domains_foreign_users"]
    ...
    async def create_go_tasking(self,
        ...
        uri = f"/api/v2/domains/{taskData.args.get_arg('object_id')}/foreign-users?limit=100"
        try:
