...
class GetGroupArguments(TaskArguments):
...
class GetGroup(CommandBase):
    cmd = "get_group"
    needs_admin = False
    help_cmd = "get_group -object_id \"S-1-5-21-909015691-3030120388-2582151266-512\""
    description = "Get information about a specific group"
    version = 1
    author = "@its_a_feature_"
    argument_class = GetGroupArguments
    supported_ui_features = ["bloodhound:get_group"]
    ...
    async def create_go_tasking(self,
        ...
        uri = f"/api/v2/groups/{taskData.args.get_arg('object_id')}?limit=100"
        try:
