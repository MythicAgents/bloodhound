...
class GetGroupMembershipsArguments(TaskArguments):
...
class GetGroupMemberships(CommandBase):
    cmd = "get_group_memberships"
    needs_admin = False
    help_cmd = "get_group_memberships -object_id \"S-1-5-21-909015691-3030120388-2582151266-512\""
    description = "Get information about which groups this group belongs to"
    version = 1
    author = "@its_a_feature_"
    argument_class = GetGroupMembershipsArguments
    supported_ui_features = ["bloodhound:get_group_memberships"]
    ...
    async def create_go_tasking(self,
        ...
        uri = f"/api/v2/groups/{taskData.args.get_arg('object_id')}/memberships?limit=100"
        try:
