...
class GetGroupMembersArguments(TaskArguments):
...
class GetGroupMembers(CommandBase):
    cmd = "get_group_members"
    needs_admin = False
    help_cmd = "get_group_members -object_id \"S-1-5-21-909015691-3030120388-2582151266-512\""
    description = "Get information about which groups/users belong to this group"
    version = 1
    author = "@its_a_feature_"
    argument_class = GetGroupMembersArguments
    supported_ui_features = ["bloodhound:get_group_members"]
    ...
    async def create_go_tasking(self,
        ...
        uri = f"/api/v2/groups/{taskData.args.get_arg('object_id')}/members?limit=100"
        try:
